...
security = release.get("securityReadiness")
validation_security = validation.get("securityReadiness")
if not isinstance(security, dict) or security != validation_security or security.get("schema") != "cloudless.security-readiness.v1":
    raise SystemExit("Signed release has invalid security readiness evidence")
required_security = release.get("channel") == "stable" and int(release.get("version", "0").split(".", 1)[0]) >= 1
if security.get("required") != required_security or security.get("status") not in {"operational", "not-operational"}:
    raise SystemExit("Signed release has invalid security readiness policy")
for field in ("version", "channel", "sourceCommit"):
    if security.get(field) != release.get(field):
        raise SystemExit(f"Signed release security readiness {field} mismatch")
if required_security and security.get("status") != "operational":
    raise SystemExit("CloudlessOS 1.0+ stable release lacks operational security ownership")
security_artifact = next(item for item in artifacts if item.get("name") == "cloudless-security-readiness.json")
