...
load_cloudless_release_env() {
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        case "$key" in
            CLOUDLESS_ARCHIVE_SECRET|CLOUDLESS_R2_ENDPOINT|CLOUDLESS_R2_BUCKET|CLOUDLESS_PHYSICAL_QUALIFICATION_DIR|CLOUDLESS_SECURITY_CONTACT_ATTESTATION|AWS_ACCESS_KEY_ID|AWS_SECRET_ACCESS_KEY)
                export "$key=$value"
                ;;
            *)
